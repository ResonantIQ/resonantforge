...
from resonantforge.cross_contamination import CrossContaminationInjector
from resonantforge.kb.generator import generate_kb
from resonantforge.layer1.coverage_backfill import CoverageBackfill
from resonantforge.layer1.plan_validator import PlanValidator, SkipRateTracker
from resonantforge.layer1.quality_plan_injector import QualityPlanInjector
from resonantforge.layer1.snapshot_emitter import SnapshotEmitter
from resonantforge.layer1.state_machine import StateMachine
from resonantforge.profiles import get_profile
from resonantforge.utils.atomic_write import (
    append_jsonl_line,
    atomic_write_jsonl,
    atomic_write_text,
    read_jsonl_robust,
...
from resonantforge.tenant_config.generator import generate_tenant_config
from resonantforge.validators.disagreement_ledger import DisagreementLedger, run_soft_judge
from resonantforge.validators.extractors.accuracy import ClaimExtractionError, extract_accuracy_signals
...
def _acquire_lock(target: Path, config: PipelineConfig) -> Path:
    """
    Write a .rforge-lock file in the parent of *target*.
    ...
    if lock_path.exists():
        try:
            ...
            if _pid_is_live(owner_pid):
                raise RuntimeError(
                    f"Another rforge process (PID {owner_pid}) is already generating "
...
def _build_conversation_signals(
    conv_id: str,
    empathy: "resonantforge.schemas.EmpathySignals",
    resolution: "resonantforge.schemas.ResolutionSignals",
    brand_voice: "resonantforge.schemas.BrandVoiceSignals",
    accuracy: "resonantforge.schemas.AccuracySignals",
) -> ConversationSignals:
    """
...
def run_pipeline(config: PipelineConfig) -> Manifest:
    ...
    Returns:
        The completed :class:`~resonantforge.schemas.Manifest` record.
    """
...
def _run_pipeline_inner(
    ...
    for w in gate_warnings:
        print(f"[rforge]   GATE WARNING: {w.message}")
        _log(config, f"  GATE WARNING: {w.message}")
    for e in gate_errors:
        print(f"[rforge]   GATE ERROR: {e.message}")
