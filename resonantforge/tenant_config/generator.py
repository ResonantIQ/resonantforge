...
The file is a serialized :class:`~resonantforge.schemas.TenantConfig` and is
...
from resonantforge.schemas import BrandVoiceVariant, CoachingStyleOverlay, TenantConfig
...
def _build_config_dict(profile: Profile) -> dict[str, Any]:
    ...
    Structure matches the :class:`~resonantforge.schemas.TenantConfig` schema:
...
def generate_tenant_config(
    ...
    ----------
    profile:
        A concrete :class:`~resonantforge.profiles.base.Profile` instance (SaaS or PS).
    output_dir:
...
def load_tenant_config(path: Path) -> dict[str, Any]:
    ...
    and is compatible with :class:`~resonantforge.schemas.TenantConfig` for Pydantic
