...
**Fix (ships in v0.2.0):** `_write_label_template` in `extractor.py` checks `label_path.exists()` and returns early if the file is already present. Never remove this guard. Re-running `extract-envelopes` is safe: envelopes are refreshed, existing `labels.json` files are left untouched.
...
**Rule:** `tags` is a closed enum enforced at load time — `load_labels` raises `LabelSchemaError` on any unknown value. To add a new tag: update `VALID_TAGS` in `resonantforge/replay/schemas.py` and the vocabulary table in `harness/docs/replay-label-schema.md` in the same PR. If you believe a new tag is needed, open a GitHub issue first (see GitHub Issues) to discuss before adding it.
